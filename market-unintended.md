...
The official writeup makes two purchases: first a normal purchase to create `HOLDING`, and then a second where `CONFIG` is supplied in place of the `HOLDING` account.

In my unintended solution, I only purchase once (instead of purchasing twice), giving me an unintended shortcut because I can corrupt ```CONFIG``` on the first attempt. 
Since `CONFIG` is already initialized, passing it as the `holding` account causes `buy()` to enter the existing-account branch and allows me to overwrite `CONFIG.owner` on the first try and get the flag with only one purchase.
